...
1. `git clone https://github.com/ArtemVoronov/clearway-task-assets-service.git`
2. `cd clearway-task-assets-service`
3. настроить среду по образу и подобию `docker-compose.yaml` (PostgreSQL с шардированными и нешардированные базами, запустить скрипты миграции `liquibase`)
...
Если базовая конфигурацияя не менялась, то приложение также будет доступно по `https://localhost:3005`
...
4.5 Несмотря на наличие поля UUID в таблице пользователей, мы сохраняем инкрементацию обычного ID - в целях удобства реализации пагинации. На текущий момент пагинация не была реализована.
...
[ ] Добавить политики ограничений для пользователей (например, максмальное количество файлов, максимальный размер на один файлы, максимальный размер всех файлов)
[ ] Добавить инетрационные тесты (скажем через отдельный `docker-compose.yml`, где через `liquibase` будут пересоздаваться базы данных в PosgreSQL для изоляции тестов)
